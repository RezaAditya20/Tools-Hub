...
PORT = 8420
HTML_FILE = os.path.join(os.path.dirname(__file__), "..", "..", "page", "Torrent-Search.html")
...
class TorrentHandler(SimpleHTTPRequestHandler):
    ...
    def _serve_html(self):
        try:
            ...
            self.send_error(404, "Torrent-Search.html not found")
            return
    ...
    def _serve_static(self, path):
        """Serve static files relative to the page/ directory."""
        page_dir = os.path.normpath(os.path.join(os.path.dirname(HTML_FILE)))
        # Resolve the file path, prevent directory traversal
        safe = path.lstrip("/")
        if ".." in safe:
            self.send_error(403)
            return
        # Try page dir first, then parent (for fontawesome etc.)
        for base in [page_dir, os.path.normpath(os.path.join(page_dir, ".."))]:
            fp = os.path.normpath(os.path.join(base, safe))
